...
function WhatsAppButton() {
  return (
    <motion.a
      href="https://wa.me/918015004952?text=Hi%20Dealpost!%20I%20want%20to%20discuss%20a%20project."
      target="_blank"
      rel="noopener noreferrer"
      className="fixed bottom-8 right-8 z-[999] w-16 h-16 bg-[#25D366] rounded-full flex items-center justify-center shadow-[0_8px_32px_rgba(37,211,102,0.4)] hover:shadow-[0_8px_48px_rgba(37,211,102,0.6)] transition-shadow"
...
function Testimonials() {
  const testimonials = [
    {
      quote: "Dealpost completely transformed our brand identity. The logo, the packaging, the social presence — everything now feels cohesive and premium.",
      name: "Arun Kumar",
      role: "Owner, Kovai Kongu Mess",
      initials: "AK"
    },
    {
      quote: "Within 3 months of working with Dealpost, our Instagram grew from 400 to 12,000 followers and our monthly orders doubled.",
      name: "Priya Suresh",
      role: "Founder, FoodBae",
      initials: "PS"
    },
    {
      quote: "The team understood our vision for Chennai Aerospace Park from day one. The brand materials they delivered are world-class.",
      name: "S. Rajendran",
      role: "Director, Chennai Aerospace Park",
      initials: "SR"
    },
    {
      quote: "Not just a design agency — Dealpost thinks strategically. They helped us position TIDEL as a global destination.",
      name: "Meena Krishnan",
      role: "Brand Manager, TIDEL Park",
      initials: "MK"
    }
  ];

  return (
    <section className="py-24 md:py-32 px-6 md:px-10 lg:px-[58px] bg-[#F3FAF7] text-[#061F1C] overflow-hidden">
      <div className="max-w-[1400px] mx-auto">
        <Reveal className="text-center mb-16">
          <span className="text-[10px] uppercase tracking-widest font-bold text-[#138F84]">CLIENT VOICES</span>
          <h2 className="text-4xl md:text-7xl font-black tracking-tighter leading-[0.9] mt-4">
            WHAT CLIENTS<br />SAY.
          </h2>
        </Reveal>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {testimonials.map((t, i) => (
            <Reveal key={i} delay={i * 0.1}>
              <div className="bg-white border border-[#138F84]/10 rounded-[32px] p-8 md:p-10 hover:border-[#138F84]/40 hover:shadow-[0_8px_40px_rgba(19,143,132,0.08)] transition-all duration-500 h-full flex flex-col justify-between">
                <div>
                  <div className="text-5xl font-black text-[#138F84] opacity-20 leading-none mb-4">"</div>
                  <p className="text-lg md:text-xl font-medium leading-relaxed text-[#061F1C] opacity-80">{t.quote}</p>
                </div>
                <div className="flex items-center gap-4 mt-8 pt-6 border-t border-[#138F84]/10">
                  <div className="w-12 h-12 rounded-full bg-[#0E544C] flex items-center justify-center text-white font-black text-sm tracking-widest">
                    {t.initials}
                  </div>
                  <div>
                    <strong className="block font-black tracking-tight text-[#061F1C]">{t.name}</strong>
                    <span className="text-[11px] uppercase tracking-widest font-bold text-[#138F84] opacity-70">{t.role}</span>
                  </div>
                </div>
              </div>
            </Reveal>
...
function Pricing() {
  const plans = [
    {
      ...
      period: "one-time",
      color: "bg-white",
      textColor: "text-[#061F1C]",
      accentColor: "text-[#138F84]",
      features: ["Logo Design", "Brand Colour Palette", "Typography System", "Business Card Design", "3 Social Media Templates"],
      cta: "Get Started",
      highlight: false
    },
    {
      ...
      accentColor: "text-[#138F84]",
      features: ["Everything in Starter", "Social Media Management", "12 Posts / Month", "Performance Ads Management", "Monthly Analytics Report", "WhatsApp Marketing"],
      cta: "Let's Talk",
      highlight: true
    },
    {
      name: "ECOSYSTEM",
      tag: "Full Stack Brand",
      price: "Custom",
      period: "quote",
      color: "bg-white",
      textColor: "text-[#061F1C]",
      accentColor: "text-[#138F84]",
      features: ["Full Branding System", "Website Development", "Custom Software / App", "Performance Marketing", "Content Production", "Dedicated Brand Manager"],
      cta: "Request Quote",
      highlight: false
    }
  ];
  ...
  return (
    <section className="py-24 md:py-32 px-6 md:px-10 lg:px-[58px] bg-[#061F1C] text-white">
      <div className="max-w-[1400px] mx-auto">
        <Reveal className="text-center mb-16">
          <span className="text-[10px] uppercase tracking-widest font-bold text-[#138F84]">TRANSPARENT PRICING</span>
          <h2 className="text-4xl md:text-7xl font-black tracking-tighter leading-[0.9] mt-4">
            INVEST IN YOUR<br /><em className="text-[#138F84] not-italic">BRAND.</em>
          </h2>
          <p className="text-lg opacity-60 mt-6 max-w-xl mx-auto">Every great brand starts with a decision. Choose a plan that fits your stage.</p>
        </Reveal>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {plans.map((plan, i) => (
            <Reveal key={i} delay={i * 0.12}>
              <div className={`${plan.color} rounded-[40px] p-8 md:p-10 flex flex-col h-full border ${plan.highlight ? 'border-[#138F84] ring-2 ring-[#138F84]/30 shadow-[0_0_60px_rgba(19,143,132,0.2)]' : 'border-white/5'} relative overflow-hidden`}>
                {plan.highlight && (
                  <div className="absolute top-6 right-6 bg-[#138F84] text-white text-[9px] font-black tracking-widest uppercase px-3 py-1 rounded-full">{plan.tag}</div>
                )}
                {!plan.highlight && (
                  <div className={`text-[9px] font-black tracking-widest uppercase ${plan.accentColor} mb-2`}>{plan.tag}</div>
                )}
                <h3 className={`text-2xl font-black tracking-widest uppercase ${plan.textColor} mt-2`}>{plan.name}</h3>
                <div className="mt-6 mb-8">
                  <span className={`text-5xl font-black tracking-tight ${plan.textColor}`}>{plan.price}</span>
                  <span className={`text-sm opacity-50 ml-2 ${plan.textColor}`}>/ {plan.period}</span>
                </div>
                <ul className="flex flex-col gap-3 flex-1">
                  {plan.features.map(f => (
                    <li key={f} className={`flex items-center gap-3 text-sm font-medium ${plan.textColor} opacity-80`}>
                      <span className="w-5 h-5 rounded-full bg-[#138F84]/20 flex items-center justify-center shrink-0">
                        <span className="w-1.5 h-1.5 rounded-full bg-[#138F84]" />
                      </span>
                      {f}
                    </li>
                  ))}
                ...
                <button
                  onClick={() => document.getElementById('contact')?.scrollIntoView({ behavior: 'smooth' })}
                  className={`mt-10 w-full py-4 rounded-2xl font-black tracking-widest uppercase text-sm transition-all duration-300 ${
                    plan.highlight
                      ? 'bg-[#138F84] text-white hover:bg-white hover:text-[#061F1C]'
                      : 'border border-[#138F84]/30 text-[#138F84] hover:bg-[#138F84] hover:text-white'
                  }`}
                >
                  {plan.cta} →
                </button>
